...
public class ConnectionServiceImpl implements ConnectionService {
    ...
    public User communicate(int senderId, int receiverId) throws Exception {
        ...
        if(user1.getMaskedIp()!=null){
            ...
            String ccCode = str.substring(0,3);

            if(ccCode.equals(user.getOriginalCountry().getCode()))
                return user;
            else {
                String countryName = "";

                if (ccCode.equalsIgnoreCase(CountryName.IND.toCode()))
                    countryName = CountryName.IND.toString();
                if (ccCode.equalsIgnoreCase(CountryName.USA.toCode()))
                    countryName = CountryName.USA.toString();
                if (ccCode.equalsIgnoreCase(CountryName.JPN.toCode()))
                    countryName = CountryName.JPN.toString();
                if (ccCode.equalsIgnoreCase(CountryName.CHI.toCode()))
                    countryName = CountryName.CHI.toString();
                if (ccCode.equalsIgnoreCase(CountryName.AUS.toCode()))
